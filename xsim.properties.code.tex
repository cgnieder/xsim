...
\cs_new_protected:Npn \__xsim_extract_property_list_entry:NNww #1#2 #3 == #4 \q_stop
  {
    \str_set:Nn #1 {#3}
    \str_set:Nn #2 {#4}
  }
...
\cs_new_protected:Npn \__xsim_remove_property_from_list:nnn #1#2#3
  {
    ...
    \xsim_foreach_list_entry:nn {#3}
      {
        \__xsim_extract_property_list_entry:NNww
          \l__xsim_tmpa_str \l__xsim_tmpb_str ##1 \q_stop
        \str_if_eq:VnT \l__xsim_tmpa_str {#1-#2}
          {
            \xsim_remove_from_list:nx {#3}
              { #1-#2 == {\l__xsim_tmpb_str} }
          }
      }
