...
\cs_new_protected:Npn \xsim_insert_solution:nn #1#2
  {
    ...
    \file_if_exist:VTF \l_xsim_file_name_tl
      {
        \xsim_verbose:n
          { Inserting~ solution~ to~ exercise~ type~ `#1'~ id~ `#2'. }
        \group_begin:
      ...
      {
        \xsim_verbose:n
          { File~ `\l_xsim_file_name_tl'~ doesn't~ exist. }
      }
  }
...
% #1: options
% #2: type
\cs_new_protected:Npn \xsim_print_type_solutions:nn #1#2
  {
    \xsim_verbose:n
      { Printing~ solutions~ for~ exercise~ type~ `#1'. }
    \group_begin:
      \bool_set_false:N \l__xsim_tmpa_bool
      \xsim_foreach_exercise_type_id:n
        {
          \xsim_if_in_list:nnT {print} {#2-##2}
            { \bool_set_true:N \l__xsim_tmpa_bool }
        }
      \tl_set:Nn \ExerciseType {#2}
...
% #1: options
\cs_new_protected:Npn \xsim_print_all_solutions:n #1
  {
    \xsim_foreach_exercise_type:n
      { \xsim_print_type_solutions:nn {#1} {##1} }
  }
