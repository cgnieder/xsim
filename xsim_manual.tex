...
\end{sourcecode}
  ...
  counter the counter will be stepped each time the exercise is used.
\end{bewareofthedog}
...
points and bonus-points to a question (see section~\ref{sec:goals} about
...
sections~\ref{sec:grad-table-templ} and~\ref{sec:template-examples}.
...
\begin{itemize}
  \item environment templates (see section~\ref{sec:envir-templ}),
  \item heading templates (see section~\ref{sec:heading-templates}) and
  \item grading table templates (see section~\ref{sec:grad-table-templ})
\end{itemize}
...
become clear in the examples in section~\ref{sec:template-examples}. Having
...
yourself in order to achieve certain layouts\footnote{I plan to incorporate
  ...
  section~\ref{sec:template-examples} but at the time of writing this is still
  up in the air.}.
...
\begin{commands}
  ...
  \command{XSIMmixedcase}[\marg{code}]
    ...
    \verbcode+\XSIMmixedcase{this is some text}+ \XSIMmixedcase{this is some text}
...
Table~\ref{tab:translation-keys} shows all existing keywords with all
