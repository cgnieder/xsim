...
\seq_new:N  \g__xsim_exercises_seq
...
% #1: boolean if fixed parameter
% #2: parameter name
\cs_new_protected:Npn \xsim_declare_parameter:nn #1#2
  {
    \xsim_verbose:n { Declaring~ parameter~ `#2'. }
    \xsim_if_parameter_exist:nF {#2}
      {
        \seq_put_right:Nn \l__xsim_parameters_seq {#2}
        \bool_if:cT {c_#1_bool}
          { \seq_put_right:Nn \l__xsim_fixed_parameters_seq {#2} }
      }
  }
...
\xsim_declare_parameter:nn {true}  {exercise-env}
\xsim_declare_parameter:nn {true}  {solution-env}
\xsim_declare_parameter:nn {false} {exercise-name}
\xsim_declare_parameter:nn {false} {solution-name}
\xsim_declare_parameter:nn {false} {exercise-template}
\xsim_declare_parameter:nn {false} {solution-template}
\xsim_declare_parameter:nn {false} {counter}
\xsim_declare_parameter:nn {true}  {number}
...
% #1: type
% #2: id
\cs_new_protected:Npn \xsim_add_exercise:nn #1#2
  { \seq_gput_right:Nn \g__xsim_exercises_seq {#1-#2} }
\cs_generate_variant:Nn \xsim_add_exercise:nn {nV}

\prg_new_conditional:Npnn \xsim_if_exercise_exists:nn #1#2 {T,F,TF}
  {
    \seq_if_in:NnTF \g__xsim_exercises_seq {#1-#2}
      { \prg_return_true: }
      { \prg_return_false: }
  }
    ...
    \xsim_if_insert_mode:F
      {
        ...
        \xsim_exercise_if_use:nVTF {#1} \g_xsim_exercise_id_tl
          {
            \xsim_add_to_list:nx {use} { #1 - \g_xsim_exercise_id_tl }
          ...
          { \xsim_remove_from_list:nx {use} { #1 - \g_xsim_exercise_id_tl } }
        \xsim_update_collections:nV {#1} \g_xsim_exercise_id_tl
        \xsim_add_exercise:nV {#1} \g_xsim_exercise_id_tl
      }
  }
    ...
    \xsim_exercise_if_print:nnTF {#1} {#2}
      { \prg_return_true: }
      {
        \bool_if:cTF {l__xsim_#1_exercise_use_bool}
          {
            \bool_if:cTF {c_ \xsim_get_property:nnn {#1} {#2} {use} _bool}
              {
...
\AtEndDocument
  {
    ...
    \xsim_update_list:n {use}
    \xsim_update_list:n {types}
    \xsim_add_cs_to_aux:nn {id} { { \int_use:N \g_xsim_id_int } }
    \xsim_foreach_exercise_type:n
      {
