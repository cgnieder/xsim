...
\begin{commands}
  \command{DeclareExerciseProperty}[\sarg\parg\narg\marg{property}]
    Declares the property \meta{property}.
    
    If used with the optional star a unique property is defined which means
    that each exercise must have a property value distinct from all other
    exercises (all means all -- \emph{independent from the exercise type}).

    If used with the optional \code{+} a boolean property is defined which
    ...
    value.  If any other value is used the property is set to \code{false}.

    If used with the optional \code{-} a property is defined which won't get
