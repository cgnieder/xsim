...
\begin{bewareofthedog}
  ...
  (\eg, \parameter{exercise-template}) that's \emph{currently active}.  If you
...
\begin{properties}
  ...
  \propbool{used}
    ...
    this is only false for exercises that have been collected (\cf\
...
\begin{bewareofthedog}
  ...
  \property{used} property.  At that time also the properties \property{page},
  \property{section} and \property{chapter} are set.
\end{bewareofthedog}
...
\section{Example Documents Coming With This Package}
The repository of this package\footnote{GitHub:
  \url{https://github.com/cgnieder/xsim/}, CTAN:
  \url{http://www.ctan.org/pkg/xsim/}} includes a number of example documents
demonstrating how different aspects of this package work or how different
kinds of problems can be solved or how different kinds of layouts can be
achieved.  They include:
\begin{itemize}
  \item \code{boxed-headings.tex} -- the headings of exercises put in a
    \pkg{tcolorbox};
  \item \code{code-and-output.tex} -- an example for the \pkg{xsimverb}
    package, see section~\vref{sec:xsimverb-package};
  \item \code{collections.tex} -- demonstrating collections, see
    section~\vref{sec:collecting-exercises};
  \item \code{crossref.tex} -- hyperlinks from exercises to solutions and
    back;
  \item \code{description-list.tex} -- exercises as items in a
    \env*{description} list;
  \item \code{different-point-types.tex} -- defining custom point types and
    using them, see section~\vref{sec:goals};
  \item \code{difficulties.tex} -- defining custom tags and using them, see
    section~\vref{sec:tags};
  \item \code{floating.tex} -- exercises as floats;
  \item \code{listings.tex} -- using code listings inside of exercises and
    solutions;
  \item \code{multiplechoice.tex} -- defining a multiple choice exercise type
    with custom properties;
  \item \code{various.tex} -- the original test file of the package author,
    demonstrating various aspects of the package.
\end{itemize}
