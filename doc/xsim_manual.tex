...
\begin{commands}
  ...
  \command{IfTagSet\TF}[\marg{value}\marg{true}\marg{false}]
    It\sinceversion{0.11} can be that want to insert text \emph{outside} of
    exercises depending on the chosen tags.  With this command you can check
    if for \property{tags} value \meta{value} has been set.
  \command{IfTopicSet\TF}[\marg{value}\marg{true}\marg{false}]
    With\sinceversion{0.11} this command you can check if for
    \property{topics} value \meta{value} has been set.
\end{commands}
