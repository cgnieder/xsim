    ...
    \xsim_if_goal_exist:nF {#1}
      {
        ...
        \xsim_declare_property:nnnn
          { \c_false_bool } { \c_false_bool } { \c_false_bool } {actual#1}
        % \xsim_make_property_alias:nn {actual#1} {#1}
...
\cs_generate_variant:Nn \xsim_synchronize_goals:nn {nV}
...
\cs_new_protected:Npn \xsim_addto_goal:nnnn #1#2#3#4
  {
    \xsim_if_goal_exist:nT {#3}
      {
        % \xsim_if_boolean_property:nnnF {#1} {#2} {printed}
        %   {
...
\cs_new_protected:Npn \xsim_temporary_goal_new:nnn #1#2#3
  {
    \fp_if_exist:cF {g__xsim_goal_#3_#1_#2_fp}
